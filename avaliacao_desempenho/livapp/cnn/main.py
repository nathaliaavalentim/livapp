...
X = []
for f in X_path:
    X.append(np.array(cv.resize(cv.imread(f), (48,48), interpolation = cv.INTER_AREA))) 
...
#TransferLearning
vgg = tf.keras.applications.VGG16(input_shape=(48,48,3), include_top = False, weights= 'imagenet')
...
#Treinando o Modelo
history = tf_model.fit(X_train, y_train, batch_size = 1, epochs = 9, initial_epoch = 0, validation_data = (X_val, y_val))
